...
<article id="bo_v" class="pt-4 pb-6">
    <header class="mb-4">
        <h2 id="bo_v_title">
            ...
            <span class="text-sm text-bold py-1 px-3 m-0 bg-indigo-100 text-indigo-500 inline-block uppercase rounded-lg mb-1 inline"><?php echo $view['ca_name']; // 분류 출력 끝 ?></span>
            <?php } ?>
        ...
        <div class="flex-none md:flex justify-between">
	        <div class="profile_info flex items-center">
	        	...
	        	<div class="profile_info_ct ml-4">
	        		<span class="sr-only">작성자</span> <strong><?php echo $view['name'] ?></strong><?php if ($is_ip_view) { echo "&nbsp;($ip)"; } ?>
	        		<div class="pt-1">
		        		...
		        		<span class="sr-only">조회</span><i class="fa fa-eye pl-2" aria-hidden="true"></i> <?php echo number_format($view['wr_hit']) ?>회
		        		<span class="if_date"><span class="sr-only">작성일</span><i class="fa fa-clock-o pl-2" aria-hidden="true"></i> <?php echo date("y-m-d H:i", strtotime($view['wr_datetime'])) ?></span>
	        		</div>
	    		</div>
	    	</div>
		    ...
		    <div id="bo_v_top" class="flex justify-end">
				...
				<ul class="btn_bo_user bo_v_com m-0 p-0">
		            ...
		            <?php if ($reply_href) { ?><li class="relative inline w-10 text-center ml-2 bg-white"><a href="<?php echo $reply_href ?>" class="btn_b01 btn inline-block text-gray-400 underline-none align-middle leading-10 py-0 px-3 text-center font-semibold border-0 text-lg  transition duration-300 transition-color ease-out" title="답변"><i class="fa fa-reply" aria-hidden="true"></i><span class="sr-only">답변</span></a></li><?php } ?>
		        	...
		        	<li class="relative inline w-10 text-center ml-2 bg-white">
		        		<button type="button" class="btn_more_opt is_view_btn btn_b01 btn inline-block text-gray-400 underline-none align-middle border-0 bg-transparent leading-10 py-0 px-3 text-center font-semibold border-0 text-lg transition duration-300 transition-color ease-out m-0 p-0"><i class="fa fa-ellipsis-v" aria-hidden="true"></i><span class="sr-only">게시판 리스트 옵션</span></button>
			        		<ul class="more_opt is_view_btn hidden block absolute top-11 right-0 bg-white border z-999">
				            <?php if ($update_href) { ?><li class="border-b p-3 float-inherit w-24 text-center m-0 text-gray-700 text-left bg-white"><a href="<?php echo $update_href ?>">수정<i class="fa fa-pencil-square-o ml-2" aria-hidden="true"></i></a></li><?php } ?>
				            <?php if ($delete_href) { ?><li class="border-b p-3 float-inherit w-24 text-center m-0 text-gray-700 text-left bg-white"><a href="<?php echo $delete_href ?>" onclick="del(this.href); return false;">삭제<i class="fa fa-trash-o ml-2" aria-hidden="true"></i></a></li><?php } ?>
				            <?php if ($copy_href) { ?><li class="border-b p-3 float-inherit w-24 text-center m-0 text-gray-700 text-left bg-white"><a href="<?php echo $copy_href ?>" onclick="board_move(this.href); return false;">복사<i class="fa fa-files-o ml-2" aria-hidden="true"></i></a></li><?php } ?>
				            <?php if ($move_href) { ?><li class="border-b p-3 float-inherit w-24 text-center m-0 text-gray-700 text-left bg-white"><a href="<?php echo $move_href ?>" onclick="board_move(this.href); return false;">이동<i class="fa fa-arrows ml-2" aria-hidden="true"></i></a></li><?php } ?>
				            <?php if ($search_href) { ?><li class="border-b p-3 float-inherit w-24 text-center m-0 text-gray-700 text-left bg-white"><a href="<?php echo $search_href ?>">검색<i class="fa fa-search ml-2" aria-hidden="true"></i></a></li><?php } ?>
				        </ul>
		        	</li>
		        	<?php } ?>
		        </ul>
    ...
    <section id="bo_v_atc">
        ...
        <div id="bo_v_share" class="relative pt-4 px-0 pb-2 my-2">
	        ...
	        <?php if ($scrap_href) { ?><a href="<?php echo $scrap_href;  ?>" target="_blank" class="btn btn_b03 py-1 px-4 text-gray-700 font-normal text-base w-full leading-7 h-7 border rounded" onclick="win_scrap(this.href); return false;"><i class="fa fa-bookmark" aria-hidden="true"></i> 스크랩</a><?php } ?>
	    </div>
        ...
        <!-- 본문 내용 시작 { -->
        <div id="bo_v_con" class="mt-2 mx-0 mb-7 w-full leading-7 min-h-48 break-all overflow-hidden"><?php echo get_view_thumbnail($view['content']); ?></div>
    ...
    <section id="bo_v_link">
        ...
        for ($i=1; $i<=count($view['link']); $i++) {
            ...
            ?>
           <li class="p-4 relative my-3 mx-0 border rounded hover:border-indigo-300 hover:text-indigo-300">
